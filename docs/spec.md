...
3. 面板顯示：prompt 輸入、風格 preset（外觀/內裝/夜景…）、控制強度滑桿（單一「保真度」滑桿，內部映射到多個 ControlNet 權重）、解析度、預估成本與預估時間。
   **並且顯示實際裁切框**：`write_image` 的取景只由輸出尺寸決定（垂直 FOV 固定、水平由長寬比推導），
   所以輸出長寬比 ≠ viewport 時，使用者在 SketchUp 看到的水平範圍不等於輸出範圍。
   以 1512×849 的視窗輸出 1024×1024，水平會被裁掉約 44%。不顯示裁切框會造成「我明明看得到那棵樹」的困惑。見 journal 002。
4. 按「Render」。
...
| 單張成本 | ≤ US$0.08（1024×1024）。全套評估 36 張約 US$1–2，含重跑抓 US$10 上限 |
...
**資料集**（依時間盒 ≤2 天，已於 open-questions Q2/Q17 縮減）
- **6 個** SketchUp 場景（3 外觀：直角量體 / 曲面 / 大面積玻璃；3 內裝：密集開口 / 細長構件如欄杆百葉 / 複雜家具）。
  選這 6 類的理由：必須涵蓋會讓結構走鐘的情境。6 個都是方盒子的話，純 img2img 也不會出錯，測不出差異。
- 每個場景 **2 個**固定相機（兩點透視 / 仰角），共 **12 shots**。
...
- 每個條件每個 shot 跑 **1 個 seed**，共 12 × 3 = **36 張**。多 seed 列為加分項。
...
- 主分析為配對比較（同 shot 同 seed 對照），對 shot 做 bootstrap（10,000 次）取 95% CI。
  註：12 shots 的 CI 會偏寬，這是時間盒的代價，報告中須明講而非隱藏。
- 回報 C−A 與 C−B 的配對差值與 CI，不只回報各自平均。
- 若之後補跑多 seed，須先在 shot 內取中位數再進 bootstrap，避免用 seed 灌大樣本數。
...
**輸出**：`eval/report.md` + 原始數據 CSV + 每個 shot 的三聯圖（A/B/C 並排）+ 2–3 個 C 表現最差的失敗案例分析。
