...
```
                         ┌──────────────┐
      使用者取消 ←────────│   created    │──────→ failed（上傳逾時 5 min / 校驗失敗）
           │             └──────┬───────┘
           │                    │ 控制圖全部上傳完成 + sha256 通過 + cost_guard 放行
           │                    ▼
           │             ┌──────────────┐
      使用者取消 ←────────│   queued     │──────→ failed    （submit 當下回 4xx）
           │             └──────┬───────┘──────→ retrying  （submit 當下回 5xx / 逾時）
           │                    │ provider 接受
           │                    ▼
           │             ┌──────────────┐
      使用者取消 ←────────│   running    │──────→ succeeded（webhook，結果已落地）
           │             └──────┬───────┘──────→ failed    （4xx，不重試）
           │                    │ 5xx / 逾時
           │                    ▼
           │             ┌──────────────┐
      使用者取消 ←────────│   retrying   │──────→ failed（retry_count 已達 2）
           │             └──────┬───────┘
           │                    │ 退避 10s → 40s 後重送
           │                    └──────────→ 回到 queued
           ▼
    ┌──────────────┐                    ┌──────────────┐
    │  cancelled   │                    │   expired    │ ← 任何非終態，created_at + 10 min
    └──────────────┘                    └──────────────┘
```

> 圖與下方的表必須一致。**表是權威** —— 若兩者打架，以表為準並修圖。
> 這張圖在 2026-09-05 補上四條邊時漏改過一次，圖比表顯眼，錯的圖比沒有圖更糟。
...
**「最多 2 次」的精確語意**
實作為 `retry_count >= 2` 時拒絕 `retrying → queued`，也就是
**允許 2 次重試、總共 3 次嘗試**。圖上若寫「重試 < 2」容易被讀成只允許 1 次。

**重試上限擋在哪一條邊**
擋在 `retrying → queued`，**不擋在進入 `retrying` 時**。
理由：花錢的是「再送一次 provider」，`retrying` 只是把「已失敗、正在決定後續」寫進軌跡。
額度用盡時讓 job 先進 `retrying`、再由 `RETRY_BUDGET_EXHAUSTED` 逼它收在 `failed`，
`job_events` 才看得出「試到最後一次才放棄」。
這也讓 `queued` 與 `running` 兩個來源的行為完全一致。

**額度釋放由誰負責**
**由呼叫端負責，不在 `job_service.transition` 內。**
這是刻意的 —— transition 只管狀態機的正確性，不該知道計費。
但代價是：每新增一條通往終態的邊，都要記得在對應的呼叫端釋放預留額度。
2026-09-05 一口氣多了三條通往終態的邊，這個代價已經浮現。
若之後漏得太頻繁，就把釋放收進 transition，並接受它變胖。

**`next_attempt_at` 在取消時不清除**
保留下來對事後分析有用（看得出「取消時本來預計何時重送」）。
但這代表 **retry sweeper 依 `next_attempt_at` 撈待重試 job 時必須同時過濾終態**，
否則會撈到已取消的 job。這條約束是實作上的硬性要求，不是選項。
