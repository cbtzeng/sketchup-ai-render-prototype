...
| 4.4 | HtmlDialog ↔ Ruby 傳遞大字串的上限與效能 | 🟢 **已實測：遠超需求** | 1K → 8M 字元逐級測試，**雙向全部完整無截斷**。1M 字元 rb2js 16 ms / js2rb 11 ms；8M 字元 127 ms / 76 ms。1024² 的 PNG 轉 base64 約 1–2 MB，遠在上限內。→ 預覽圖可走 base64；但 `file://` 仍為預設（少一次編解碼與記憶體複製），base64 作為備案。bridge 的上限取實測值的 1/4～1/2 留餘裕 |
...
| # | 項目 | 信心 | 說明 |
|---|---|---|---|
| 4.8.1 | **`Face#pushpull` 會刪掉原本那個 Face** | 🟢 **已實測** | 它不是「把面推進去」，而是建立新幾何並取代原面。在 pushpull 之後對原 Face 設材質會拋 `TypeError: reference to deleted DrawingElement`。**正確順序是先設材質再 pushpull**（新幾何會繼承），之後若還要碰原物件必須用 `valid?` 守衛 |

這一項是寫評估場景腳本時踩到的。記錄下來的理由與本文件其他項目相同：
**「看起來理所當然」的 API 行為，不驗證就會在最不方便的時候咬人。**
